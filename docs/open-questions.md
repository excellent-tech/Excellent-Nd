...
> ChatGPT → Plan / Task分割 / 人間による実行承認（Human GO） → GitHub Issues → Symphony → Codex → GitHub Result → 人間の明示的な取り込み → ChatGPT
...
- Symphony未導入hostのbootstrapは人間による実行承認（Human GO）とIssue記録を必須とし、通常routing条件を付けない限定例外とする。runtime / profileのsmoke verification後、別の通常Taskでsingle Task E2Eを行う。
...
- version は validated stable / development を分離し、stable は latest へ自動追従しない。強制更新は 1.0.x 全回帰テストを必須とする。
...
| 人間判断ゲート（Human Gate） 実装 | control label を外す / 戻す運用で running agent の停止と continuation が安定するか | 保留 / 再開 |
